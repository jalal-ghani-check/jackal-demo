...
var jackal_nodejsProtos = require('@jackallabs/jackal.nodejs-protos');
...
class MnemonicWallet {
    ...
    static async create(mnemonic) {
        let directWallet = await protoSigning.DirectSecp256k1HdWallet.fromMnemonic(mnemonic, {
            prefix: 'jkl'
        });
        let aminoWallet = await amino.Secp256k1HdWallet.fromMnemonic(mnemonic, {
            prefix: 'jkl'
        });
        /* Destroy mnemonic */
        mnemonic = '';
...
async function hashAndHex(input) {
    const algo = 'SHA-256';
    const raw = await node_crypto.webcrypto.subtle.digest(algo, new TextEncoder().encode(input));
 ...
 */
function genIv() {
    return node_crypto.getRandomValues(new Uint8Array(16));
}
/**
...
async function aesCrypt(data, key, iv, mode) {
    ...
    if (data.byteLength < 1) {
        return node_buffer.Buffer.from([]);
    }
    else if (mode?.toLowerCase() === 'encrypt') {
        return await subtle
            .encrypt(algo, key, data)
            .then((res) => {
            return node_buffer.Buffer.from(res);
        })
            .catch((err) => {
...
async function aesToString(wallet, pubKey, aes) {
    const theIv = wallet.asymmetricEncrypt(node_buffer.Buffer.from(aes.iv), pubKey);
    const key = await exportJackalKey(aes.key);
    const theKey = wallet.asymmetricEncrypt(node_buffer.Buffer.from(key), pubKey);
    return `${theIv}|${theKey}`;
}
/**
...
async function convertToPublicFile(workingFile) {
    ...
    const detailsBuf = node_buffer.Buffer.from(JSON.stringify(details));
    const publicArray = [
        node_buffer.Buffer.from((detailsBuf.length + 16).toString().padStart(8, '0')),
        detailsBuf
    ];
    for (let i = 0; i < workingFile.size; i += chunkSize) {
        const bufChunk = node_buffer.Buffer.from(await workingFile.slice(i, i + chunkSize).arrayBuffer());
        publicArray.push(node_buffer.Buffer.from((bufChunk.length + 16).toString().padStart(8, '0')), bufChunk);
    }
...
async function convertFromPublicFile(source) {
    let detailsBuf = node_buffer.Buffer.from('');
    const bufParts = [];
...
async function convertToEncryptedFile(workingFile, key, iv) {
    ...
    for (let i = 0; i < workingFile.size; i += chunkSize) {
        const bufChunk = node_buffer.Buffer.from(await workingFile.slice(i, i + chunkSize).arrayBuffer());
        encryptedArray.push(node_buffer.Buffer.from((bufChunk.length + 16).toString().padStart(8, '0')), 
...
async function cryptString(input, key, iv, mode) {
    if (mode === 'encrypt') {
        const result = await aesCrypt(node_buffer.Buffer.from(input), key, iv, mode);
        return result.toString('base64');
    }
    else if (mode === 'decrypt') {
        const result = await aesCrypt(node_buffer.Buffer.from(input, 'base64'), key, iv, mode);
...
async function standardizeBlob(source) {
    return new node_buffer.Blob([new Uint8Array(await source.arrayBuffer())]);
}
/**
...
async function saveFileTreeEntry(toAddress, rawPath, rawTarget, rawContents, walletRef) {
    ...
    const msg = {
        ...
        trackingNumber: node_crypto.randomUUID(),
        editors: '',
        viewers: ''
    };
        ...
        if (fileProviders && fileProviders.length) {
            ...
            for (let i = 0; i < fileProviders.length; i++) {
                ...
                try {
                    ...
                    return await FileDownloadHandler.trackFile(node_buffer.Buffer.from(chunks), key, iv);
                }
                catch (err) {
                    const attempt = i + 1;
        ...
        if (fileProviders && fileProviders.length) {
            for (let i = 0; i < fileProviders.length; i++) {
                ...
                try {
                    ...
                    return await PublicFileDownloadHandler.trackFile(node_buffer.Buffer.from(rawFile));
                }
                catch (err) {
                    const attempt = i + 1;
                    const remaining = fileProviders.length - attempt;
...
class QueryHandler {
    ...
    static async trackQuery(queryUrl) {
        const builder = new jackal_nodejsProtos.QueryBuilder(queryUrl);
...
class ProtoHandler extends QueryHandler {
    ...
    static async trackProto(cfg) {
        const builder = new jackal_nodejsProtos.ProtoBuilder(cfg.signer, cfg.rpcUrl, cfg.queryUrl);
...
class WalletHandler {
    ...
    asymmetricEncrypt(toEncrypt, pubKey) {
        return eciesjs.encrypt(pubKey, toEncrypt).toString('hex');
    }
    /**
    ...
    asymmetricDecrypt(toDecrypt) {
        ...
        return eciesjs.decrypt(this.properties.keyPair.toHex(), node_buffer.Buffer.from(toDecrypt, 'hex'));
    }
    /**
...
async function processWallet(config, session) {
    ...
    const secretAsHex = bufferToHex(node_buffer.Buffer.from(secret, 'base64').subarray(0, 32));
    const keyPair = eciesjs.PrivateKey.fromHex(secretAsHex);
    const isDirect = protoSigning.isOfflineDirectSigner(signer);
    const properties = {
        signer,
        keyPair,
        rnsInitComplete,
...
exports.AbciHandler = AbciHandler;
...
exports.GovHandler = GovHandler;
exports.MnemonicWallet = MnemonicWallet;
exports.NotificationHandler = NotificationHandler;
exports.OracleHandler = OracleHandler;
exports.RnsHandler = RnsHandler;
exports.StorageHandler = StorageHandler;
exports.WalletHandler = WalletHandler;
exports.aesCrypt = aesCrypt;
exports.aesToString = aesToString;
exports.blockToDate = blockToDate;
exports.blockToDateFixed = blockToDateFixed;
exports.bruteForceString = bruteForceString;
exports.checkResults = checkResults;
exports.convertFromEncryptedFile = convertFromEncryptedFile;
exports.convertFromPublicFile = convertFromPublicFile;
exports.convertToEncryptedFile = convertToEncryptedFile;
exports.convertToPublicFile = convertToPublicFile;
exports.cryptString = cryptString;
exports.deprecated = deprecated;
exports.exportJackalKey = exportJackalKey;
exports.genIv = genIv;
exports.genKey = genKey;
exports.getAverageBlockTime = getAverageBlockTime;
exports.getFileTreeData = getFileTreeData;
exports.getRandomIndex = getRandomIndex;
exports.handlePagination = handlePagination;
exports.importJackalKey = importJackalKey;
exports.numTo3xTB = numTo3xTB;
exports.numToWholeTB = numToWholeTB;
exports.orderStrings = orderStrings;
exports.setDelay = setDelay;
exports.signerNotEnabled = signerNotEnabled;
exports.standardizeBlob = standardizeBlob;
exports.standardizeFileSource = standardizeFileSource;
exports.stringToAes = stringToAes;
exports.stringToUint16 = stringToUint16;
exports.stringToUint8 = stringToUint8;
exports.stripper = stripper;
exports.uint16ToString = uint16ToString;
